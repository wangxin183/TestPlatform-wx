...
**产品主线**仅四页：**需求分析 / 用例生成 / App UI 执行 / Skills 管理**。架构与后端约束以 `CLAUDE.md` 为准；此处写工作方式 + 展示侧边界。
...
## 1. 主线页面：前端约束（RA / TCG / EXE / Skills）

后端产品规则见 `CLAUDE.md` 对应章节。前端**只展示与触发**，不重复实现业务决策。  
侧栏仅这四项；`/` 重定向到需求分析。已移除：仪表盘、项目管理、需求管理、用例管理、用例评审、旧版执行。

### 1.1 需求分析页

- 路由：`/requirement-analysis`；脚本：`requirement_analysis.js`
...
- 用例评审（通过/驳回/编辑/重编译）在本页完成，不依赖已删除的「用例管理/用例评审」页
...
- 路由：`/app-execution`；脚本：`app_execution.js`；改完递增 `?v=`
...
- 路由：`/skills`；脚本：`skills.js`
- 仅管理 Skill 资源；勿在此页实现 RA/TCG/EXE 业务逻辑
...
- [ ] 若动 RA/TCG/EXE/Skills：未在前端实现组批、入口导航、错误码死表或执行状态机；未恢复已删除的仪表盘/项目管理等入口
