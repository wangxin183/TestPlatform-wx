...
需求拆解阶段只允许使用下方“需求文档”正文。**知识库内容不得进入 FR/NFR、验收条件或需求描述**；即使知识库中存在相关功能，也不能据此补充、扩写或推断需求。
...
**范围边界（必须遵守）**：
- **全文动态识别**需求范围，不依赖“第三章”或固定 `3.x` 编号。模块可能出现在任意章节、任意标题层级，也可能没有模块标题。
- 优先从“详细功能要求 / 详细需求 / 功能需求 / 需求说明”及其具体条目提取；无固定结构时，逐段识别明确、可执行的需求陈述。
- **禁止**仅凭产品概述、功能总览、产品定位表、目标用户、修订记录生成 FR；这些内容只作背景。
- **禁止**从知识库、历史笔记或行业惯例生成、补齐、扩写 FR/NFR。
...
- **acceptance_criteria**：只能把原文明示的行为、状态、条件改写为可验证表达；原文没有说明的跳转、展示条件、布局、文案或业务规则不得自行补充
...
- `source_evidence` 至少包含一段可以在上传文档正文中逐字匹配的原文摘录；知识库文本不能作为 evidence。
...
- 不得根据通用性能指标、OWASP、行业基线或知识库自行创建 NFR；这些只能放入测试建议，不能冒充文档需求。
...
3. **不要扩写范围**：概述里提到但无「详细功能要求」的模块（如轻小说/社区/首页），不要生成 FR
